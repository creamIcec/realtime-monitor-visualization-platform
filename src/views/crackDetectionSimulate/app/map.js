...
class WebMap {
  ...
  display = function () {
    if (this.map != null) {
      ...
      this.switchDayNight(this.displayMode);
      return 0
    } else {
  ...
  animatePath = function (refresh, forceStop) {
    if (forceStop && this.trackAnimation != null) {
  ...
  addWarningPos = function(point){

  }

  setWarningIcon = function(imgPath){
    const size = 24;
    this.warningIcon = new BMapGL.Icon(imgPath, new BMapGL.Size(size, size), {
  ...
  createNewWarning = function(point){
    console.log("创建警告:" + point);
    let baiduPoint = new BMapGL.Point(point.getLongitude(), point.getLatitude());
    this.warningMarker = new BMapGL.Marker(baiduPoint, { icon: this.warningIcon });
    this.map.addOverlay(this.warningMarker);
    this.warningMarker.addEventListener('click', () => {
      const alertPanel = document.getElementById("crack-info-panel");
      alertPanel.style.visibility = 'visible';
    });
    //this.warningMarker.setPosition(baiduPoint);
  }
